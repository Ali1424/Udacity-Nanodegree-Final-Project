...
pipeline { environment {
    registry = "udacitydevopsali/udacity_capstone_project"
            ...
            steps{
                script {
                    withCredentials([string(credentialsId: 'dockerhub_password', variable: 'MY_PASSWORD')]) {
                            sh "echo '${MY_PASSWORD}' | docker login --username udacitydevopsali --password-stdin"
                        }
                    // sh "echo ""$MY_PASSWORD"" | docker login --username udacitydevopsali --password-stdin"

                    // sh script: 'docker login --docker login --username udacitydevopsali --password-stdin', stdin: "$MY_PASSWORD"
